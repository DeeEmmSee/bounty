...
BountyContribution.getMyContributionsCount = function(userID) {
    return new Promise(function(success, fail) {
        sql.query("SELECT COUNT(1) as 'Total' FROM bountycontributions bc LEFT JOIN bounties b ON b.ID = bc.BountyID WHERE bc.UserID = ? AND Paid = FALSE AND (b.Status = 1 OR b.Status = 2)", userID, function(err, res) {
            if (err) {
                console.log(err);
                fail(err);
            }
